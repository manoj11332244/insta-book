...
class PostListView(ListView):
    ...
    def get_queryset(self):
        return Post.objects.filter(approve=True)
...
class UserPostListView(ListView):
    ...
    def get_queryset(self):
        ...
        return Post.objects.filter(author=user, approve=True).order_by('-date_posted')
...
def search(request):
    ...
    if len(query) >= 150 or len(query) < 1:
        ...
        allpostsTitle = Post.objects.filter(title__icontains=query, approve=True)
        allpostsAuthor = Post.objects.filter(author__username = query, approve=True)
